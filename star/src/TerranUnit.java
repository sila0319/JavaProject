...
abstract class TerranUnit extends StarObject implements IStarUnit {
	...
	public void attack(int power,int attackspeed,ArrayList attacker2vt,StarObject target) { //공격한 유닛의 파워, 공격당할 그릇, 공격 당할 타겟
		while(true){
			...
			try{
				if(target.energy<=0){ //타겟의 피가 0이하가 될때까지 공격
					...
					attacker2vt.removeElement(target); //타겟의 피가 0이되면 제거
					break;
