...
import ddd.changeSCV;
import ddd.changeStarUI;
import ddd.changeTerranBuilding;

class CommandCenter extends changeTerranBuilding  implements Runnable{
	static int cost=400;
	static int gascost=0;
	static int buildtime=500;
	CommandCenter(changeStarUI ui){
		super(ui);
		name = "CommandCenter";
		flag=true;
		work=true;
	...
	public void makeUnit(){
		if(ui.clan.getMineral()>=SCV.cost){
			if(ui.clan.getMaxunit()>ui.clan.getUnit()){ //최대유닛보다 현재 유닛이 적을 경우
				...
				ui.clan.workerVT.add(new changeSCV(ui));
