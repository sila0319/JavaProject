package Gold5;
	...
	public class 톱니바퀴_14891 {
		...
		static void check(int n) {//톱니바퀴가 돌아갈 수 있는 조건을 체크한다. 
			...
			for (int i = n+1; i <4; i++) {//n을 기준으로 오른쪽에 위치한 톱니바퀴들을 체크한다.
	...
	arr = new int[4][8];//톱니바퀴는 4개와 8개의 s,n극을 가진다. 
	...
	for (int i = 0; i < k; i++) {//k번만큼 톱니바퀴를 회전한다.
	    ...
	    spincan= new int [4];//spincan배열을 새롭게 초기화 시킨다.
	    spincan[n]=p;//n번째의 톱니바퀴 상태는 p로 지정해준다. 
	    check(n);//톱니바퀴 체크
	    spin();//돌리기
	}

	for (int i = 0; i < 4; i++) {//점수 계산하는 부분.
	   if(arr[i][0]==1) {
	      if(i==0) {
	         result +=1;
	      }else if(i==1) {
