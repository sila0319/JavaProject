...
public class 윷놀이_2490 {
	public static void main(String[] args) {
		...
		for (int i = 0; i < 3; i++) {//3번 결과보기
			result =0;//값초기화
			for (int j = 0; j < 4; j++) {//윷4개던지기
				int n = sc.nextInt();
				result +=n;
			}
			if(result==4) {//윷4개의 합 4면 모
				System.out.println("E");
			}else if(result==3) {//윷4개의 합 3이면 도
				System.out.println("A");
			}else if(result==2) {//윳4개의 합 2면 개
				System.out.println("B");
			}else if(result==1) {// 윷 4개의 합 1면 걸
				System.out.println("C");
			}else if(result==0){// 윷4개의 합 0이면 윷
